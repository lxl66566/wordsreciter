...
url_settingswidget::url_settingswidget(QString text[4], QString urls[4],QWidget *parent) :
    QDialog(parent),
    ui(new Ui::url_settingswidget)
{
    ui->setupUi(this);
    ...
    {
        emit ok_url(ui->engurl1->text(),ui->engurl2->text(),ui->japurl1->text(),ui->japurl2->text());
        emit ok_text(ui->engtext1->text(),ui->engtext2->text(),ui->japtext1->text(),ui->japtext2->text());
        close();
    });
...
url_settingswidget::~url_settingswidget()
{
    delete ui;
}
